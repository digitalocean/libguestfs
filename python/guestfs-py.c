...
py_guestfs_test0 (PyObject *self, PyObject *args)
{
  ...
  int optargs_t_obool = -1;
  int optargs_t_oint = -1;
  long long optargs_t_oint64 = -1;
  const char *optargs_t_ostring = NULL;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "OszOiiLsss#iiLz:guestfs_test0",
                         &py_g, &str, &optstr, &py_strlist, &b, &integer, &integer64, &filein, &fileout, &bufferin, &bufferin_size, &optargs_t_obool, &optargs_t_oint, &optargs_t_oint64, &optargs_t_ostring))
    return NULL;
  g = get_handle (py_g);
  strlist = get_string_list (py_strlist);
  if (!strlist) return NULL;

  if (optargs_t_obool != -1) {
...
static PyObject *
py_guestfs_test0rbufferout (PyObject *self, PyObject *args)
{
  PyThreadState *py_save = NULL;
  PyObject *py_g;
  guestfs_h *g;
  ...
  if (!PyArg_ParseTuple (args, (char *) "Os:guestfs_test0rbufferout",
                         &py_g, &val))
    return NULL;
  g = get_handle (py_g);
...
static PyObject *
py_guestfs_test0rbufferouterr (PyObject *self, PyObject *args)
{
  PyThreadState *py_save = NULL;
  PyObject *py_g;
  guestfs_h *g;
  ...
  if (!PyArg_ParseTuple (args, (char *) "O:guestfs_test0rbufferouterr",
                         &py_g))
    return NULL;
  g = get_handle (py_g);
...
py_guestfs_mount_local (PyObject *self, PyObject *args)
{
  ...
  int optargs_t_cachetimeout = -1;
  int optargs_t_debugcalls = -1;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "Osizii:guestfs_mount_local",
                         &py_g, &localmountpoint, &optargs_t_readonly, &optargs_t_options, &optargs_t_cachetimeout, &optargs_t_debugcalls))
    return NULL;
  g = get_handle (py_g);
...
py_guestfs_umount_local (PyObject *self, PyObject *args)
{
  ...
  int r;
  int optargs_t_retry = -1;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "Oi:guestfs_umount_local",
                         &py_g, &optargs_t_retry))
    return NULL;
  g = get_handle (py_g);

  if (optargs_t_retry != -1) {
...
py_guestfs_tune2fs (PyObject *self, PyObject *args)
{
  ...
  const char *device;
  int optargs_t_force = -1;
  int optargs_t_maxmountcount = -1;
  int optargs_t_mountcount = -1;
  const char *optargs_t_errorbehavior = NULL;
  long long optargs_t_group = -1;
  int optargs_t_intervalbetweenchecks = -1;
  int optargs_t_reservedblockspercentage = -1;
  const char *optargs_t_lastmounteddirectory = NULL;
  long long optargs_t_reservedblockscount = -1;
  long long optargs_t_user = -1;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "OsiiizLiizLL:guestfs_tune2fs",
                         &py_g, &device, &optargs_t_force, &optargs_t_maxmountcount, &optargs_t_mountcount, &optargs_t_errorbehavior, &optargs_t_group, &optargs_t_intervalbetweenchecks, &optargs_t_reservedblockspercentage, &optargs_t_lastmounteddirectory, &optargs_t_reservedblockscount, &optargs_t_user))
    return NULL;
  g = get_handle (py_g);
...
py_guestfs_md_create (PyObject *self, PyObject *args)
{
  ...
  PyObject *py_devices;
  char **devices;
  long long optargs_t_missingbitmap = -1;
  int optargs_t_nrdevices = -1;
  int optargs_t_spare = -1;
  long long optargs_t_chunk = -1;
  const char *optargs_t_level = NULL;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "OsOLiiLz:guestfs_md_create",
                         &py_g, &name, &py_devices, &optargs_t_missingbitmap, &optargs_t_nrdevices, &optargs_t_spare, &optargs_t_chunk, &optargs_t_level))
    return NULL;
  g = get_handle (py_g);
  devices = get_string_list (py_devices);
  if (!devices) return NULL;
...
py_guestfs_e2fsck (PyObject *self, PyObject *args)
{
  ...
  int r;
  const char *device;
  int optargs_t_correct = -1;
  int optargs_t_forceall = -1;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "Osii:guestfs_e2fsck",
                         &py_g, &device, &optargs_t_correct, &optargs_t_forceall))
    return NULL;
  g = get_handle (py_g);

  if (optargs_t_correct != -1) {
    optargs_s.correct = optargs_t_correct;
    optargs_s.bitmask |= GUESTFS_E2FSCK_CORRECT_BITMASK;
  }
  if (optargs_t_forceall != -1) {
...
py_guestfs_ntfsfix (PyObject *self, PyObject *args)
{
  ...
  int r;
  const char *device;
  int optargs_t_clearbadsectors = -1;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "Osi:guestfs_ntfsfix",
                         &py_g, &device, &optargs_t_clearbadsectors))
    return NULL;
  g = get_handle (py_g);

  if (optargs_t_clearbadsectors != -1) {
...
py_guestfs_ntfsclone_out (PyObject *self, PyObject *args)
{
  ...
  int optargs_t_metadataonly = -1;
  int optargs_t_rescue = -1;
  int optargs_t_ignorefscheck = -1;
  int optargs_t_preservetimestamps = -1;
  int optargs_t_force = -1;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "Ossiiiii:guestfs_ntfsclone_out",
                         &py_g, &device, &backupfile, &optargs_t_metadataonly, &optargs_t_rescue, &optargs_t_ignorefscheck, &optargs_t_preservetimestamps, &optargs_t_force))
    return NULL;
  g = get_handle (py_g);

  if (optargs_t_metadataonly != -1) {
    optargs_s.metadataonly = optargs_t_metadataonly;
    optargs_s.bitmask |= GUESTFS_NTFSCLONE_OUT_METADATAONLY_BITMASK;
  }
  if (optargs_t_rescue != -1) {
    optargs_s.rescue = optargs_t_rescue;
    optargs_s.bitmask |= GUESTFS_NTFSCLONE_OUT_RESCUE_BITMASK;
  }
  if (optargs_t_ignorefscheck != -1) {
    optargs_s.ignorefscheck = optargs_t_ignorefscheck;
    optargs_s.bitmask |= GUESTFS_NTFSCLONE_OUT_IGNOREFSCHECK_BITMASK;
  }
  if (optargs_t_preservetimestamps != -1) {
...
py_guestfs_mkfs_btrfs (PyObject *self, PyObject *args)
{
  ...
  PyObject *py_devices;
  char **devices;
  long long optargs_t_allocstart = -1;
  long long optargs_t_bytecount = -1;
  const char *optargs_t_datatype = NULL;
  int optargs_t_leafsize = -1;
  const char *optargs_t_label = NULL;
  const char *optargs_t_metadata = NULL;
  int optargs_t_nodesize = -1;
  int optargs_t_sectorsize = -1;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "OOLLzizzii:guestfs_mkfs_btrfs",
                         &py_g, &py_devices, &optargs_t_allocstart, &optargs_t_bytecount, &optargs_t_datatype, &optargs_t_leafsize, &optargs_t_label, &optargs_t_metadata, &optargs_t_nodesize, &optargs_t_sectorsize))
    return NULL;
  g = get_handle (py_g);
  devices = get_string_list (py_devices);
  if (!devices) return NULL;
...
py_guestfs_set_e2attrs (PyObject *self, PyObject *args)
{
  ...
  const char *file;
  const char *attrs;
  int optargs_t_clear = -1;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "Ossi:guestfs_set_e2attrs",
                         &py_g, &file, &attrs, &optargs_t_clear))
    return NULL;
  g = get_handle (py_g);

  if (optargs_t_clear != -1) {
...
py_guestfs_btrfs_fsck (PyObject *self, PyObject *args)
{
  ...
  int r;
  const char *device;
  long long optargs_t_superblock = -1;
  int optargs_t_repair = -1;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "OsLi:guestfs_btrfs_fsck",
                         &py_g, &device, &optargs_t_superblock, &optargs_t_repair))
    return NULL;
  g = get_handle (py_g);
...
static PyMethodDef methods[] = {
  ...
  { (char *) "test0rbufferout", py_guestfs_test0rbufferout, METH_VARARGS, NULL },
  { (char *) "test0rbufferouterr", py_guestfs_test0rbufferouterr, METH_VARARGS, NULL },
