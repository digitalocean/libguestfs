...
static PyObject *
put_int_bool_list (struct guestfs_int_bool_list *int_bools)
{
  PyObject *list;
  int i;

  list = PyList_New (int_bools->len);
  for (i = 0; i < int_bools->len; ++i)
    PyList_SetItem (list, i, put_int_bool (&int_bools->val[i]));
  return list;
};
...
static PyObject *
put_statvfs_list (struct guestfs_statvfs_list *statvfss)
{
  PyObject *list;
  int i;

  list = PyList_New (statvfss->len);
  for (i = 0; i < statvfss->len; ++i)
    PyList_SetItem (list, i, put_statvfs (&statvfss->val[i]));
  return list;
};
