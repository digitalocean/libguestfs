...
C<$LIBGUESTFS_QEMU> or using the L</guestfs_set_qemu> API (note that qemu is
...
handle for convenience; qemu is not involved.  Your Linux distro may provide
C<vmlinux>, or you may need to compile it yourself from the kernel source
(which is dead easy, follow the instructions here:
...
important shortcomings.  The main ones are:
