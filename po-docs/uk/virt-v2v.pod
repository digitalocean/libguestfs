...
=head1 КОРОТКИЙ ОПИС
 ...
 virt-v2v -ic qemu:///system гостьова_система_qemu --in-place

=head1 ОПИС

Virt-v2v converts guests from a foreign hypervisor to run on KVM.  It can
read Linux and Windows guests running on VMware, Xen, Hyper-V and some other
hypervisors, and convert them to KVM managed by libvirt, OpenStack, oVirt,
Red Hat Virtualisation (RHV) or several other targets.
...
to a RHV-M Export Storage Domain which is located remotely (over NFS) at
C<rhv.nfs:/export_domain>.  If you are unclear about the location of the
...
For more information about converting from VMX files see L</INPUT FROM
VMWARE VMX> below.
...
Див. L</ВИВЕДЕННЯ ДО GLANCE> нижче.

=head2 Перетворити образ диска на образ диска
...
=head1 РЕЖИМИ ВВЕДЕННЯ І ВИВЕДЕННЯ
...
from VMware, RHEL 5 Xen and more.  The I<-ic> option selects the precise
libvirt source.
...
I<-o rhv> is used to write to a RHV / oVirt target.  I<-o vdsm> is only used
when virt-v2v runs under VDSM control.
...
is qcow2 (see I<-of> below), and is equivalent to the I<-c> option of
L<qemu-img(1)>.
...
as analyzing the ovf file, and then convert the guest.  See L</INPUT FROM
VMWARE OVA> below
...
See also L</INPUT FROM VMWARE VCENTER SERVER>, L</INPUT FROM XEN> below.
...
Див. L</ПЕРЕТВОРЕННЯ НА МІСЦІ> нижче.

Конфліктує з усіма параметрами I<-o *>.

=item B<-it> B<ssh>

When using I<-i vmx>, this enables the ssh transport.  See L</INPUT FROM
VMWARE VMX> below.

=item B<-it> B<vddk>

Use VMware VDDK as a transport to copy the input disks.  See L</INPUT FROM
VDDK> below.  If you use this parameter then you may need to use other
I<--vddk*> options to specify how to connect through VDDK.
...
=item B<-n> введення:виведення

=item B<-n> виведення

=item B<--network> введення:виведення

=item B<--network> виведення

=item B<-b> введення:виведення

=item B<-b> виведення

=item B<--bridge> введення:виведення

=item B<--bridge> виведення
...
is uploaded to Glance.  See L</OUTPUT TO GLANCE> below.
...
Див. L</ВИВЕДЕННЯ ДО LIBVIRT> нижче.
...
L</OUTPUT TO LIBVIRT> below for further information.
...
transport.  See L</INPUT FROM VDDK> below for details.
...
L</INPUT FROM VDDK> below for details.

=item B<--vddk-config> НАЗВА_ФАЙЛА

=item B<--vddk-cookie> КУКА

=item B<--vddk-nfchostport> ПОРТ

=item B<--vddk-port> ПОРТ

=item B<--vddk-snapshot> SNAPSHOT-MOREF

=item B<--vddk-transports> РЕЖИМ:РЕЖИМ:...

=item B<--vddk-vimapiver> APIVER

When using VDDK mode, these options are passed unmodified to the
L<nbdkit(1)> VDDK plugin.  Please refer to L<nbdkit-vddk-plugin(1)>.  These
are all optional.
...
If I<--vdsm-compat=1.1> is used then modern qcow2 (I<compat=1.1>)  files are
generated instead.

Currently I<--vdsm-compat=0.10> is the default, but this will change to
I<--vdsm-compat=1.1> in a future version of virt-v2v (when we can assume
...
=head1 ПАРАВІРТУАЛІЗОВАНІ ГОСТЬОВІ СИСТЕМИ XEN
...
Guests which use the Windows E<ge> 8 "Fast Startup" feature (or guests which
...
To map a specific bridge to a target network, for example C<br0> on the
source to C<ovirtmgmt> on the target, use:
...
VMWARE VMX> below), or if that is not possible then see L</INPUT FROM VMWARE
ESXi HYPERVISOR>.
...
If the username contains a backslash (eg. C<DOMAIN\USER>) then you will need
to URI-escape that character using C<%5c>: C<DOMAIN%5cUSER> (5c is the
...
connects directly to the server.  Use I<--password-file> to supply a
password via a file.
...
To create an OVA in vSphere, use the "Export OVF Template" option (from the
VM context menu, or from the File menu).  Either "Folder of files" (OVF) or
"Single file" (OVA) will work, but OVA is probably easier to deal with.  OVA
files are really just uncompressed tar files, so you can use commands like
C<tar tf VM.ova> to view their contents.
...
If you exported the guest as a "Folder of files", I<or> if you unpacked the
OVA tarball yourself, then you can point virt-v2v at the directory
...
You should use the OVA or VMX methods above (see L</INPUT FROM VMWARE OVA>
and/or L</INPUT FROM VMWARE VMX>) if possible, as they are much faster and
require much less disk space than the method described in this section.

You can use the L<virt-v2v-copy-to-local(1)> tool to copy the guest off the
hypervisor into a local file, and then convert it.

=head2 ESXi: ВИЛУЧЕННЯ ІНСТРУМЕНТІВ VMWARE З ГОСТЬОВИХ СИСТЕМ WINDOWS
...
=head2 ESXi: КОПІЮВАННЯ ГОСТЬОВОЇ СИСТЕМИ НА ЛОКАЛЬНИЙ КОМП'ЮТЕР

Using the libvirt URI as the I<-ic> option, copy one of the guests to the
local machine:

 $ virt-v2v-copy-to-local -ic esx://root@esxi.example.com?no_verify=1 guest

Ця команда створить F<guest.xml>, F<guest-disk1>, ...
...
You must also compile nbdkit, enabling the VDDK plugin.  nbdkit E<ge> 1.1.25
...
F<nbdkit> which runs the locally built copy of nbdkit and its plugins.  So
set C<$PATH> to point to the nbdkit top build directory (that is, the
...
You must find the SSL "thumbprint" of your VMware server.  How to do this is
explained in L<nbdkit-vddk-plugin(1)>, also available at the link above.
...
=head2 VDDK: ІМПОРТУВАННЯ ГОСТЬОВОЇ СИСТЕМИ
...
I<--vddk-config>, I<--vddk-cookie>, I<--vddk-nfchostport>, I<--vddk-port>,
I<--vddk-snapshot>, I<--vddk-transports> and I<--vddk-vimapiver>, which are
all explained in the L<nbdkit-vddk-plugin(1)> documentation.

=head2 VDDK: ДІАГНОСТУВАННЯ ПОМИЛОК VDDK
...
Virt-v2v is able to import Xen guests from RHEL 5 Xen or SLES and openSUSE
Xen hosts.
...
=head2 XEN: ІМПОРТУВАННЯ ГОСТЬОВОЇ СИСТЕМИ
...
Вивантаження можливе лише до гіпервізора KVM.
...
C<glance image-show> will display the metadata as "Property" fields such as
C<os_type> and C<hw_disk_bus>.
...
=head2 Glance і робота з декількома дисками
...
=head2 Імпортування дисків до Cinder

Since most virt-v2v guests are "pets", Glance is perhaps not the best place
to store them.  There is no way for virt-v2v to upload directly to Cinder
(L<https://bugzilla.redhat.com/1155229>).  There are two ways to upload to
Cinder:
...
Import the image to Glance first (ie. I<-o glance>) and then copy it to
Cinder:

 cinder create --image-id <UUID-ОБРАЗУ-GLANCE> <РОЗМІР>

=item 2.

Create (through some other means) a new volume / LUN in your Cinder backing
store.  Migrate the guest to this volume (using I<-o local>).  Then ask
...
=head2 Обчислювальні потужності і обсяг оперативної пам’яті
...
=head1 ЗАВДАННЯ ПІСЛЯ ПЕРЕТВОРЕННЯ

=head2 Налаштовування гостьової мережі
...
=head2 Перетворення гостьової системи Windows
...
=head1 ВІЛЬНЕ МІСЦЕ ДЛЯ ПЕРЕТВОРЕННЯ

=head2 Вільне місце у гостьовій системі
...
=head2 Мінімальний обсяг місця у основній системі
...
=head1 МІНІМАЛЬНИЙ XML ДЛЯ ПАРАМЕТРА -i libvirtxml
...
and contents, but using virtual devices more appropriate for KVM
...
C<qemu:///system> libvirt instance, and apply various fixups to the guest OS
...
(F<> if unset).  It can be a directory I<or> point to F<virtio-win.iso> (CD
ROM image containing drivers).

Див. L<ВМИКАННЯ VIRTIO>.
...
followed by running C<virt-v2v -i libvirtxml> to perform the conversion.
