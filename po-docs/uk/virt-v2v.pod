...
Red Hat Enterprise Virtualisation (RHEV) or several other targets.
...
Встановити метод виведення до I<rhev>.
...
Див. L</ВИВЕДЕННЯ ДО RHEV> нижче.

=item B<-o> B<vdsm>

Встановити метод виведення до I<vdsm>.

This mode is similar to I<-o rhev>, but the full path to the data domain
must be given:
...
Currently KVM supports OVMF, a partially open source UEFI firmware, and can
run these guests.
...
To copy the disks, virt-v2v uses L<libcurl(3)>.  The Curl library obeys the
proxy environment variables, in particular C<https_proxy>, C<all_proxy> and
C<no_proxy> (C<HTTPS_PROXY>, C<ALL_PROXY> and C<NO_PROXY> can also be used,
but the lowercase named environment variables take precedence).
...
is managed by VDSM using the I<-o vdsm> output mode (which end users should
...
stands for "Storage Pool Manager").  This is a RHEV node that is elected to
...
or RHEV Export Storage Domain to another.  It only permits importing a guest
that was previously exported from another oVirt/RHEV instance.
...
RHEV.  For more information, see this blog posting by the author of
virt-v2v:
