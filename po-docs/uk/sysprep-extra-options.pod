...
=item B<--delete> delete (see C<delete> below)

Delete (recursively) the specified C<path> on guest.

=item B<--firstboot> firstboot (see C<firstboot> below)
...
=item B<--password> password (see C<password> below)
...
=item B<--password КОРИСТУВАЧ:file:НАЗВА_ФАЙЛА>

Change the password for C<USERNAME>.  Read the password from C<FILENAME>.
...
=item B<--password КОРИСТУВАЧ:password:ПАРОЛЬ>

Change the password for C<USERNAME>.  Set the password to the literal string
C<PASSWORD>.
...
=item B<--password-crypto> спосіб_шифрування_паролів (див. C<password> нижче)

Встановити спосіб шифрування паролів: C<md5>, C<sha256> або C<sha512>.

Для шифрування за допомогою C<sha256> і C<sha512> потрібна glibc E<ge> 2.7
(див. L<crypt(3)> у гостьовій системі).
...
=item B<--root-password> root-password (see C<password> below)
