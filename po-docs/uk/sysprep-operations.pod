...
An alternative option, I<--scrub>, can be used to scrub files.  This only
...
операційної системи, наприклад C<yum update>.

Виведені дані і повідомлення про помилки для таких скриптів буде записано до
C<~root/virt-sysprep-firstboot.log> (у гостьовій системі).

=head3 Зауваження щодо firstboot
...
=head3 Зауваження щодо hostname
...
=head3 Зауваження щодо password
...
=head3 Зауваження щодо user-account

У поточній версії записи користувачів не вилучаються з
C</etc/shadow>. Причиною є те, що у Augeas ще немає модуля для файлів
паролів shadow.
