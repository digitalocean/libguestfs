...
аргументів. Див. L</ДОДАТКОВІ АРГУМЕНТИ>.
...
Ця команда отримує вміст файла F<назва_файла> і записує його доло кального
...
Ця команда повертах розмір сектора на блоковому пристрої. Зазвичай, розміром
...
Повертає назву виконуваного файла поточного гіпервізора.
...
Докладніший опис архітектури libguestfs навденено у підручнику з
L<guestfs(3)>.
...
This is different from L</tmpdir>, as we need shorter paths for sockets (due
to the limited buffers of filenames for UNIX sockets), and L</tmpdir> may be
too long for them.
...
Ця команда завершує роботу гіпервізора.
...
зберігаються у каталозі C<шлях>. Значенням аргумента C<назви> є список
...
які зберігаються у каталозі C<шлях>. Значенням аргумента C<назви> є список
файлів у цьому каталозі.
...
Вказати виконуваний файл гіпервізора (зазвичай qemu), який буде використано.
...
This command runs F<file> after first decompressing C<path> using C<method>.
