...
=item B<--list --list-format> формат

=item B<--list --long>

Показати список доступних шаблонів.
...
=head3 Визначення розкладки клавіатури за допомогою C</etc/sysconfig/keyboard>
...
C<~/.ssh> directory to find the default public ID file.  That key is
...
Read the ssh key from C<FILENAME>.  C<FILENAME> is usually a I<.pub> file.
...
C<~USER/.ssh/authorized_keys> file will be created if not existing already.
...
$XDG_CONFIG_HOME/virt-builder/repos.d/ (C<$XDG_CONFIG_HOME> дорівнює
C<$HOME/.config>, якщо не встановлено).
...
L<curl(1)>, L<virt-make-fs(1)>, L<genisoimage(1)>, L<fixfiles(8)>,
