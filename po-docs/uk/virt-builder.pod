...
наведеного вище прикладу буде F<fedora-20.img>.  Змінити назву файла
...
Те саме, що і вище, але із записом результатів у форматі qcow2 до
F<fedora-20.qcow2>.
...
Те саме, що і вище, але із застосуванням шаблону i386, якщо він є доступним.

=head2 Встановлення пароля root

 virt-builder fedora-20 --root-password file:/tmp/rootpw

Створити образ Fedora 20. Пароль root буде визначено за файлом
F</tmp/rootpw>.

Зауважте, що якщо ви I<не встановите> I<--root-password>, гостьовій системі
буде надано I<випадковий> пароль адміністратора root.
...
Для оновлення основних пакунків до останньої версії виконайте такі дії:
...
у результаті команду L<yum(8)> C<update> буде один раз виконано під час
...
мережі.  (Див. L<guestfs(3)/BACKEND>).
...
Virt-builder виконує синхронізацію fsync файла результатів або образу диска,
якщо такий існує.
...
=head3 Встановлення японської мови для Fedora 20
...
=head3 Встановлення японської мови для Debian 7 (Wheezy)
...
У Linux, BSD та інших гостьових системах.
...
Перевірено підпис шаблону.

=item *

Шаблон розпаковано до файла tmp.
...
Долучено додаткові диски (I<--attach>).

=item *

Створено нову базу псевдовипадкових чисел для гостьової системи.
...
Зміну міток SELinux виконано (I<--selinux-relabel>).
...
use I<--write> (to point the package manager at a repo)  followed by
I<--install> (to install from that repo).  The I<--write> and I<--install>
steps run in the opposite order, regardless of their order on the command
line.  You have to do this using I<--run-command> instead of I<--install>.
...
Щоб увімкунути модуль UML, ознайомтеся із настановами у розділі
...
L<virt-customize(1)>, L<virt-install(1)>, L<virt-rescue(1)>,
L<virt-resize(1)>, L<virt-sysprep(1)>, L<oz-install(1)>, L<gpg(1)>,
L<curl(1)>, L<virt-make-fs(1)>, L<genisoimage(1)>,
