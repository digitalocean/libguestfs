...
Wipe C<builder.log> (log file from build commands) from the image after
...
[B<Note:> Use of nbdkit is disabled because of a libvirt bug:
L<https://bugzilla.redhat.com/show_bug.cgi?id=1011063> It will be enabled in
a future release.]
...
L<virt-sysprep(1)>, L<oz-install(1)>, L<nbdkit(1)>, L<nbdkit-xz-plugin(1)>,
L<gpg(1)>, L<curl(1)>, L<http://libguestfs.org/>.
