...
the API.  The F<src> subdirectory contains source for the library.  The
...
concentrate on first? F<src> and F<daemon> which contain the source code of
...
F<m4/guestfs_*.m4>.  Most of the configure script is split over many m4
macro files by topic, for example F<m4/guestfs_daemon.m4> deals with the
...
=head1 ДОДАВАННЯ НОВОЇ ПРИВ’ЯЗКИ ДО МОВИ
...
=head1 ТЕСТУВАННЯ ВНЕСЕНИХ ВАМИ ЗМІН
...
=head1 ПІДТРИМКА ІНТЕРНАЦІОНАЛІЗАЦІЇ (I18N)
...
=head1 СТВОРЕННЯ СТАБІЛЬНОГО ВИПУСКУ
...
L<guestfs-internals(3)>, L<guestfs-performance(1)>,
