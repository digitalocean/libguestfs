...
=head4 Функція C<lib/create.c:disk_create_qcow2>
...
=head4 Функція C<lib/drives.c:valid_port>
 ...
 /**
  * Перевіряємо, чи коректною є назва вузла сервера.
  */
 ...
 /**
  * Перевіряємо, чи коректним є номер порту.
  */
 static int
 valid_port (int port)
...
=head4 Структура C<lib/guestfs-internal.h:guestfs_h>
 ...
 struct guestfs_h {
   ...
   char *hv;			/* Виконуваний файл гіпервізора (HV). */
   char *append;			/* Додаток до командного рядка ядра. */
 
   struct hv_param *hv_params;   /* Додаткові параметри гіпервізора. */
...
Додадє завершальний NULL наприкінці вектора.
...
way this works is complicated but hinted at here:
L<http://git.qemu.org/?p=qemu.git;a=blob;f=util/keyval.c;h=93d5db6b590427e412dfb172f1c406d6dd8958c1;hb=HEAD>

=item *

Some options are syntactic sugar.  eg. C<-kernel foo> is sugar for
C<-machine kernel=foo>.
...
This is like C<qemuopts_add_arg> except that no quoting is done on the
value.
...
This is turned into a comma-separated list, like: C<-drive
file=foo,if=ide>.  Note that this handles qemu quoting properly, so
individual elements may contain commas and this will do the right thing.
...
програм та переховлення виведених ними даних.
...
Опис аргумента C<flags> наведено у описі функції C<commandrvf>.
...
cannot use libvirt for the reasons outlined in this message:
...
Get the output of lscpu as a list of (key, value) pairs (as a flattened list
of strings).
...
Create the F<physical.xml> file, which is a piece of phony libvirt XML used
to communicate the metadata of the physical machine to virt-v2v.
...
example if the RTC is 1 hour ahead of UTC, this will return C<3600>.  This
is stored in C<config-E<gt>rtc_offset>.
...
Return RTC offset from UTC in seconds, positive numbers meaning that the RTC
is running ahead of UTC.

In the error case, C<rtcE<gt>offset> is updated with 0 and C<rtcE<gt>basis>
is set to C<BASIS_UNKNOWN>.
