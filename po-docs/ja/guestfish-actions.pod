...
Limit the size of a subvolume which's path is C<subvolume>. C<size> can have
suffix of G, M, or K.
...
All entries in the filesystem are returned, excluding C<.> and C<..>. This
function can list deleted or unaccessible files.  The entries are I<not>
sorted.
...
Filesystem reference number of the node. It migh be C<0> if the node has
been deleted.
...
File size in bytes. It migh be C<-1> if the node has been deleted.
