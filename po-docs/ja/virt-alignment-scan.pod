...
デバイス名およびパーティション名 (例: C</dev/sda1> は最初のブロックデバイスにある最初のパーティションを意味します)。

すべての libvirt ドメインを一覧表示するとき (I<-a> や I<-d> オプションなし)、この列は libvirt 名または UUID
...
すべての libvirt ドメインを一覧表示するとき (I<-a> や I<-d>
オプションが指定されていないとき)、このオプションのみが適用されます。
