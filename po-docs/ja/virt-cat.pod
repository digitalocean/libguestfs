...
C<mydomain> という名前の libvirt 仮想マシンの中にある C</etc/fstab> を表示します:
...
mountpoints needed to work with the filenames gives as arguments.
...
C<virt-cat> は Windows のドライブレターとパスの解釈能力に制限があります（例: C<E:\foo\bar.txt>）。
...
ここで、C<disk.img> はディスクイメージ、C</dev/sda1> はディスクイメージにあるファイルシステム、そして C<file>
はファイルの完全パスです。
