...
An RPC with C<FileOut> parameters (eg. L<guestfs(3)/guestfs_upload>).  We
...
An RPC with C<FileIn> parameters (eg. L<guestfs(3)/guestfs_download>).  We
...
Both C<FileOut> and C<FileIn> parameters.  There are no calls like this in
...
=head4 Function C<fish/inspect.c:make_mapname>
...
=head4 Function C<fish/inspect.c:inspect_do_decrypt>
...
=head4 Function C<fish/options.c:display_short_options>
...
=head4 Function C<fish/options.c:display_long_options>
...
=head3 File F<python/guestfs-py-byhand.c>

This file contains a small number of functions that are written by hand.
The majority of the bindings are generated (see F<python/guestfs-py.c>).
