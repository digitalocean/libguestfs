...
libguestfs won't modify the file.  (See also L</DISK IMAGE FORMATS> below).
...
handle files contain any ASCII NUL (C<\0>) characters.  However the API is
very simple to use.
...
Only a subset of the attach-method backends support hotplugging (currently
only the libvirt attach-method has support).  It also requires that you use
libvirt E<ge> 0.10.3 and qemu E<ge> 1.2.
...
The libvirt attach-method requires that you set the C<format> parameter of
L</guestfs_add_drive_opts> accurately when you use writable NBD disks.

=item *

The libvirt attach-method has a bug that stops Unix domain socket
connections from working:
L<https://bugzilla.redhat.com/show_bug.cgi?id=922888>

=item *

The appliance attach-method does not support readonly connections because of
a bug in qemu: L<https://bugs.launchpad.net/qemu/+bug/1155677>
...
回避策は L</guestfs_mount_options> を使用し、実際に使用したいマウントオプションを設定することです。
...
attach-method has been selected.  Libvirt connections may require
authentication, for example if they need to access a remote server or to
access root services from non-root.  Libvirt authentication happens via a
callback mechanism, see
