...
=item B<--user-accounts> USERS (see C<user-account> below)

The user accounts to be removed (or not) from the guest.  The value of this
option is a list of user names separated by comma, where specifying an user
means it is going to be removed, while prepending C<-> in front of it name
means it is not removed.  For example:
