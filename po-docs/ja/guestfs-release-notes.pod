...
UPDATED TO 350dfe1fa26015fb0631abc3d3dc01b78dee8ead
...
Improvements to guestfish tab completion (Pino Toscano).
...
Libguestfs now compiles on Mac OS X (Margaret Lewicka).
...
Set the UUID of a filesystem to a randomly generated value (Chen Hanxiao).
...
C<guestfs_set_uuid> can set UUID of swap partitions, XFS, Btrfs (Hu Tao,
Chen Hanxiao).
...
Translations have moved from Transifex to Zanata.
...
The huge F<src/guestfs.c> file is now split into smaller logical units.
