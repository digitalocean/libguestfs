...
libguestfs tree.  As part of the refactoring, virt-p2v has been rewritten
(from Ruby, now C), and virt-v2v now uses several modern features of qemu
which make it run many times faster than before.  L<virt-v2v(1)> can import
from: VMware vCenter, VMware OVA files, local disk files, libvirt, RHEL 5
Xen.  It can output to: oVirt or Red Hat Enterprise Virtualization (RHEV-M),
OpenStack Glance, libvirt.
...
logs, debian-installer files, apt logs, exim logs, ConsoleKit logs, and
several other log files.  The virt-sysprep C<user-account> operation is now
able to remove or keep particular user accounts.  (Pino Toscano).
...
=head1 libguestfs 1.20 のリリースノート
