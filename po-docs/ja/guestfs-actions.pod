...
its own.  See L</CALLS WITH OPTIONAL ARGUMENTS>.
 ...
 GUESTFS_ADD_DRIVE_OPTS_SERVER, const char *server,
 GUESTFS_ADD_DRIVE_OPTS_PORT, int port,
...
For protocols which require access to a remote server, this is the name of
the server.

=item C<port>

For protocols which require access to a remote server, this is the port
number of the service.

If not specified, this defaults to the standard port for the protocol,
eg. 10809 when C<protocol> is C<"nbd">.
...
C<guestfs_available> および L<guestfs(3)/AVAILABILITY> 参照。
...
C<guestfs_available>, L<guestfs(3)/AVAILABILITY> 参照。
...
drive: see L<guestfs(3)/HOTPLUGGING>.  The disk B<must not> be in use
