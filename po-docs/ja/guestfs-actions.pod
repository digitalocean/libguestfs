...
このコマンドはファイルの一部を読み込みます。C<device> を C<offset> から開始して C<count> バイト読み込みます。
...
アプライアンスを起動するために libvirt を使用します。オプションの I<URI> は使用する libvirt 接続 URI です
(L<http://libvirt.org/uri.html> 参照)。
