...
この関数はオプション引数なしで C<guestfs_add_drive_opts>
...
C<param> 文字列の最初の文字は C<-> (ダッシュ) である必要があります。

C<value> は NULL もありえます。
...
このコマンドはおそらく対話式のセッションにおいて有用です。プログラムはおそらく代わりに C<guestfs_readdir> を使用するでしょう。
...
I<この関数は推奨されません。> 新しいコードでは、代わりに L</guestfs_mkfs_opts> を使用してください。
...
I<この関数は推奨されません。> 新しいコードでは、代わりに L</guestfs_ntfsresize_opts> を使用してください。
