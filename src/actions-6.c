                            ...
                            int enable)
{
  ...
  if (guestfs___check_appliance_up (g, "btrfs_quota_enable") == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
  ...
  args.fs = (char *) fs;
  args.enable = enable;
  serial = guestfs___send (g, GUESTFS_PROC_BTRFS_QUOTA_ENABLE,
                           progress_hint, 0,
  ...
  r = guestfs___recv (g, "btrfs_quota_enable", &hdr, &err,
        NULL, NULL);
  if (r == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
  ...
  if (guestfs___check_reply_header (g, &hdr, GUESTFS_PROC_BTRFS_QUOTA_ENABLE, serial) == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
                       "btrfs_quota_enable", "-1");
    return -1;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (errnum <= 0)
      ...
      guestfs___error_errno (g, errnum, "%s: %s", "btrfs_quota_enable",
                           err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return -1;
                             ...
                             const char *path)
{
  ...
  if (guestfs___check_appliance_up (g, "btrfs_qgroup_remove") == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
  ...
  args.dst = (char *) dst;
  args.path = (char *) path;
  serial = guestfs___send (g, GUESTFS_PROC_BTRFS_QGROUP_REMOVE,
                           progress_hint, 0,
  ...
  r = guestfs___recv (g, "btrfs_qgroup_remove", &hdr, &err,
        NULL, NULL);
  if (r == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
  ...
  if (guestfs___check_reply_header (g, &hdr, GUESTFS_PROC_BTRFS_QGROUP_REMOVE, serial) == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
                       "btrfs_qgroup_remove", "-1");
    return -1;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (errnum <= 0)
      ...
      guestfs___error_errno (g, errnum, "%s: %s", "btrfs_qgroup_remove",
                           err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return -1;
