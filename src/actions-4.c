                           ...
                           const char *path)
{
  ...
  if (guestfs___check_appliance_up (g, "btrfs_scrub_start") == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
                       "btrfs_scrub_start", "-1");
    return -1;
  ...
  args.path = (char *) path;
  serial = guestfs___send (g, GUESTFS_PROC_BTRFS_SCRUB_START,
                           progress_hint, 0,
  ...
  r = guestfs___recv (g, "btrfs_scrub_start", &hdr, &err,
        NULL, NULL);
  if (r == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
  ...
  if (guestfs___check_reply_header (g, &hdr, GUESTFS_PROC_BTRFS_SCRUB_START, serial) == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
                       "btrfs_scrub_start", "-1");
    return -1;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (errnum <= 0)
      ...
      guestfs___error_errno (g, errnum, "%s: %s", "btrfs_scrub_start",
                           err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return -1;
                           ...
                           int partnum)
{
  ...
  if (guestfs___check_appliance_up (g, "part_get_gpt_guid") == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
  ...
  args.device = (char *) device;
  args.partnum = partnum;
  serial = guestfs___send (g, GUESTFS_PROC_PART_GET_GPT_GUID,
                           progress_hint, 0,
  ...
  r = guestfs___recv (g, "part_get_gpt_guid", &hdr, &err,
  ...
  if (guestfs___check_reply_header (g, &hdr, GUESTFS_PROC_PART_GET_GPT_GUID, serial) == -1) {
    if (trace_flag)
      guestfs___trace (g, "%s = %s (error)",
                       "part_get_gpt_guid", "NULL");
    return NULL;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (errnum <= 0)
      ...
      guestfs___error_errno (g, errnum, "%s: %s", "part_get_gpt_guid",
                           err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return NULL;
