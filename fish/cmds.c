...
static void print_int_bool_list (struct guestfs_int_bool_list *int_bools)
{
  int i;

  for (i = 0; i < int_bools->len; ++i)
    print_int_bool (&int_bools->val[i]);
}

static void print_lvm_pv (struct guestfs_lvm_pv *lvm_pv)
{
  int i;
...
static void print_lvm_pv_list (struct guestfs_lvm_pv_list *lvm_pvs)
{
  ...
  for (i = 0; i < lvm_pvs->len; ++i)
    print_lvm_pv (&lvm_pvs->val[i]);
}

static void print_lvm_vg (struct guestfs_lvm_vg *lvm_vg)
{
  int i;
...
static void print_lvm_vg_list (struct guestfs_lvm_vg_list *lvm_vgs)
{
  ...
  for (i = 0; i < lvm_vgs->len; ++i)
    print_lvm_vg (&lvm_vgs->val[i]);
}

static void print_lvm_lv (struct guestfs_lvm_lv *lvm_lv)
{
  int i;
  ...
  printf ("lv_kernel_major: %" PRIi64 "\n", lvm_lv->lv_kernel_major);
  printf ("lv_kernel_minor: %" PRIi64 "\n", lvm_lv->lv_kernel_minor);
  printf ("lv_size: %" PRIu64 "\n", lvm_lv->lv_size);
  printf ("seg_count: %" PRIi64 "\n", lvm_lv->seg_count);
  printf ("origin: %s\n", lvm_lv->origin);
  if (lvm_lv->snap_percent >= 0) printf ("snap_percent: %g %%\n", lvm_lv->snap_percent);
  else printf ("snap_percent: \n");
  if (lvm_lv->copy_percent >= 0) printf ("copy_percent: %g %%\n", lvm_lv->copy_percent);
...
static void print_statvfs_list (struct guestfs_statvfs_list *statvfss)
{
  int i;

  for (i = 0; i < statvfss->len; ++i)
    print_statvfs (&statvfss->val[i]);
}
