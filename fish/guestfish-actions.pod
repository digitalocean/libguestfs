...
This command cannot do partial copies (see L</copy-size>).
...
This command is only needed because of L</resize2fs>
(q.v.).  Normally you should use L</fsck>.
...
L</mount> in any code that needs performance, and instead
use L</mount-options> (use an empty string for the first
...
I<Note:> It is sometimes required that you run L</e2fsck-f>
